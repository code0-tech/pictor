...
export class DFlowDataTypeContainsTypeRule {
    public static validate(value: RawDataTypes, config: DFlowDataTypeContainsTypeRuleConfig, service: DFlowDataTypeService): boolean {
        if (!(Array.isArray(value))) return false
        if (!service.getDataType(config.type)) return false
        return value.every(value1 => service.getDataType(config.type)?.validateValue(value))
    }
}