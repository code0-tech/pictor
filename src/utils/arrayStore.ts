 ...
 */
export type ArrayStore<T> = [T[], React.Dispatch<React.SetStateAction<T[]>>]

export class ArrayService<T> {

    protected readonly store: ArrayStore<T>

    constructor(store: ArrayStore<T>) {
        this.store = store
    }
...
// @ts-ignore
export const createArrayService = <K, T extends ArrayService<K>>(service: typeof T, callback?: (store: ArrayStore<K>) => T, initial?: K[]): [K[] | undefined, T] => {
