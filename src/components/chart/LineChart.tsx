...
type LineChartComponent = <TData extends Record<string, unknown>>(
...
export const LineChart: LineChartComponent = (props) => {
  ...
  const resolvedConfig = React.useMemo<ChartConfig>(() => {
    ...
    return lines.reduce((acc, line) => {
      const lineColor =
        line.color ??
        (typeof line.stroke === "string" ? line.stroke : undefined);
      acc[line.dataKey] = {
  ...
  return (
    <ChartContainer config={resolvedConfig} {...containerProps}>
      <RechartsPrimitive.LineChart data={data} {...rest}>
        ...
        {showXAxis && (
          <RechartsPrimitive.XAxis dataKey={xKey} {...xAxisProps} />
        )}
        ...
        {lines.map((line) => {
          ...
          const resolvedStroke =
            ...
            `var(--color-${dataKey}, currentColor)`;
